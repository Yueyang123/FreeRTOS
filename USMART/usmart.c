...
//使能定时器4,使能中断.
//TIM4时钟来自APB1,当APB1的分频系数不为1的时候，定时器频率=APB1*2
//因此TIM4的工作频率是200Mhz
void Timer4_Init(u16 arr,u16 psc)
{
	RCC->APB1LENR|=1<<2;   	//TIM4 时钟使能      
 	TIM4->ARR=arr;  		//设定计数器自动重装值  
	TIM4->PSC=psc;  		//预分频器20000,得到10Khz的计数时钟 
	TIM4->DIER|=1<<0;   	//允许更新中断	 
	TIM4->CR1|=0x01;    	//使能定时器4
  	MY_NVIC_Init(3,0,TIM4_IRQn,NVICPRIORITYGROUP);//抢占3，子优先级3，组2(组2中优先级最低的)									 
}
#endif
////////////////////////////////////////////////////////////////////////////////////////
//初始化串口控制器
//tsclk:定时器的时钟源（Mhz）
void usmart_init(u8 tsclk)
{
#if USMART_ENTIMX_SCAN==1
	Timer4_Init(1000,(u32)tsclk*200-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
