...
void Fatfs_Init(void)
{
    ...
    sprintf(tbuf,"SD Total Size:  %d  MB\r\n",total>>10);
	printf(tbuf);
    sprintf(tbuf,"SD FREE Size:  %d  MB\r\n",free>>10);//显示SD卡剩余容量 MB
    printf(tbuf);
     while(exf_getfree("1:",&total,&free))	//得到SD卡的总容量和剩余容量
	{
        printf("FLASH Card Fatfs Error!");
	}  
    sprintf(tbuf,"FLASH Total Size:  %d  MB\r\n",total>>10);
	printf(tbuf);
    sprintf(tbuf,"FLASH FREE Size:  %d  MB\r\n",free>>10);//显示SD卡剩余容量 MB
    printf(tbuf);
}
