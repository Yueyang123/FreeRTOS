...
u8 GT9XX_IIC_RADDR= 0xBB;			// IIC初始化地址
...
void Touch_IIC_GPIO_Config (void)
{
	...
	GPIO_Pin_Set(Touch_IIC_SCL_PORT, Touch_IIC_SCL_PIN, 1);		// SCL输出高电平
	GPIO_Pin_Set(Touch_IIC_SDA_PORT, Touch_IIC_SDA_PIN, 1);    // SDA输出高电平
	GPIO_Pin_Set(Touch_INT_PORT, 	  Touch_INT_PIN,     0);  // INT输出低电平
	GPIO_Pin_Set(Touch_RST_PORT,     Touch_RST_PIN,     1);    // RST输出高电平
...
uint8_t Touch_IIC_WaitACK(void)
{
	...
	if( GPIO_Pin_Get(Touch_IIC_SDA_PORT,Touch_IIC_SDA_PIN) != 0) //判断设备是否有做出响应		
	{
		Touch_IIC_SCL(0);	
		Touch_IIC_Delay( IIC_DelayVaule );		
		return ACK_ERR;	//无应答
...
void GT9XX_Reset(void)
{
	...
	// 初始化引脚状态
	GPIO_Pin_Set(Touch_INT_PORT,Touch_INT_PIN,0);  // INT输出低电平
	GPIO_Pin_Set(Touch_RST_PORT,Touch_RST_PIN,1);    // RST输出高	电平
	Touch_IIC_Delay(10000);
	
	// 开始执行复位
	//	INT引脚保持低电平不变，将器件地址设置为0XBA/0XBB
	GPIO_Pin_Set(Touch_RST_PORT,Touch_RST_PIN,0); // 拉低复位引脚，此时芯片执行复位
	Touch_IIC_Delay(150000);			// 延时
	GPIO_Pin_Set(Touch_RST_PORT,Touch_RST_PIN,1);			// 拉高复位引脚，复位结束
