 ...
 * @Date: 2021-08-21 07:42:40
 * @LastEditors: YURI
 * @LastEditTime: 2021-08-21 08:17:35
 */
#include "ADC.h"
#include "delay.h"		 
//初始化ADC
...
//初始化ADC3 
//我们默认仅开启ADC3_CH18P(内部温度传感器所在通道) CH17P（VBAT/4）
//16位转换时间位:采样周期+8.5个ADC周期
void  Adc3_Init(void)
{     
	...
	ADC3->SMPR2&=~(7<<(3*7));	//通道17采样时间清空	  
 	ADC3->SMPR2|=7<<(3*7); 		//通道17 810.5个周期,提高采样时间可以提高精确度		
...
{
    u32 adcx;
    adcx=Get_Adc3_Average(ADC3_BATTARY,10);
    return 4*3.3*(float)adcx/4096;
}
