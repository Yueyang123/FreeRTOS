...
//升级说明
//20190215  V1.1
//1,将SDIO的时钟源改为:pll2_r_ck, 以减少CRC错误的概率
//2,修改SDMMC_INIT_CLK_DIV和SDMMC_TRANSFER_CLK_DIV, 以匹配时钟源的改变
////////////////////////////////////////////////////////////////////////////////// 	 
...
SD_Error SD_Init(void)
{
	...
	GPIO_Set(GPIOC,0X1F<<8,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_FAST,GPIO_PUPD_PU);	//PC8,9,10,11,12复用功能输出	
 	GPIO_Set(GPIOD,1<<2,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_FAST,GPIO_PUPD_PU);		//PD2复用功能输出	
	...
	//如果直接使用pll1_q_ck时钟源,则操作SD卡时,非常容易出现CRC错误.
	//换成pll2_r_ck的时候,结果会好很多,因此我们改为来自于pll2_r_ck
	RCC->D1CCIPR&=~(1<<16);		//清除SDMMCSEL原来的设置
	RCC->D1CCIPR|=1<<16;		//SDMMCSEL=1,SDMMC时钟来自于pll2_r_ck		

 	MY_NVIC_Init(0,0,SDMMC1_IRQn,2);	//SDMMC中断配置
...
//SDMMC时钟初始化设置
//clkdiv:时钟分频系数
//CK时钟=sdmmc_ker_ck/[2*clkdiv];(sdmmc_ker_ck钟一般为200Mhz)
