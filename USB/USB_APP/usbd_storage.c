...
#define STORAGE_BLK_NBR                  2048	// 块数量
#define STORAGE_BLK_SIZ                  4096	// 每个块大小4096
...
{     
	switch(lun)
	{
		case 0://SPI FLASH
            ...
            *block_size = STORAGE_BLK_SIZ;	//SPI FLASH的前面25M字节,文件系统用
			break;
		case 1://SD卡
			*block_size=512;  
...
int8_t STORAGE_Read (uint8_t lun,uint8_t *buf,uint32_t blk_addr,uint16_t blk_len)
{
	...
	{
		case 0://SPI FLASH
            SPI_W25Qxx_ReadBuffer( (uint8_t *)buf, blk_addr<<12, blk_len<<12 );   // 读取数据，最大不能超过flash芯片的大小
			break;
		case 1://SD卡
